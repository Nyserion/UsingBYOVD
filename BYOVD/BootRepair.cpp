...
BOOLEAN
BootRepair::Initialize() noexcept
{
	if (m_bInitialized)
	{
		return TRUE;
	...
	if (!FileUtils::CreateDriverFile(driverFullPath,
		(const char*)BootRepairBin::hexData,
	...
	std::string serviceName = "BootRepair";
...
HANDLE BootRepair::CreateDevice()
{
	auto hDevice = CreateFileA("\\\\.\\BootRepair",
